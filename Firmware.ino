...
{
  const uint8_t leftLed = pgm_read_byte(&SPIN_LEDS_LEFT[0]);
  const uint8_t rightLed = pgm_read_byte(&SPIN_LEDS_RIGHT[0]);
...
void showDrumlineLane(const uint8_t lane[])
{
  ...
  for (uint8_t position = 0; position < DRUMLINE_LANE_LENGTH; ++position) {
    const uint8_t leftLed = pgm_read_byte(&SPIN_LEDS_LEFT[position]);
    const uint8_t rightLed = pgm_read_byte(&SPIN_LEDS_RIGHT[position]);
